...
zbc.createWorker({
    ...
    taskHandler: job => {
        ...
        log(`Page exists in object: ${pageName ? Object.prototype.hasOwnProperty.call(confluencePages, pageName) : 'false (no pageName)'}`)
...
zbc.createWorker({
    ...
    taskHandler: job => {
        ...
        log(`Looking for page: "${pageName}"`)
        log(`Page exists in object: ${pageName ? Object.prototype.hasOwnProperty.call(spPages, pageName) : 'false (no pageName)'}`)
    ...
    taskHandler: async job => {
        ...
        try {
            ...
            if (found) {
                ...
                log(`✅ SSL Labs analysis complete! Found ${tableCount} tables with total ${allTables.reduce((sum, table) => sum + table.length, 0)} rows`)
